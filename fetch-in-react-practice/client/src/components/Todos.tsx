...
export function Todos() {
  ...
  useEffect(() => {
    async function getTodos() {
      ...
      try {
        ...
        if (!res.ok)
          throw new Error(`Get todos error: ${res.status} ${res.statusText}`);
        const responseTodos = await res.json();
  ...
  async function addTodo(newTodo: UnsavedTodo) {
    ...
    try {
      ...
      if (!res.ok)
        throw new Error(`Add todo failed: ${res.status} ${res.statusText}`);
      const resNewTodo = await res.json();
      setTodos((todos) => [...todos, resNewTodo]);
    } catch (err) {
      console.error(err);
      setError(err);
    }
  ...
  async function toggleCompleted(todo: Todo) {
    ...
    const newObj = { task: todo.task, isCompleted: !todo.isCompleted };
    try {
      ...
      setTodos((todos) =>
        todos.map((value) =>
          value.todoId === todo.todoId ? resNewTodo : value
        )
      );
    } catch (err) {
      console.error(err);
